...
describe("CategoriesPage", () => {
  beforeEach(() => {
    ...
    request.mockResolvedValueOnce([
      ...
      { id: "category-2", user_id: "user-1", name: "월급", kind: "income" },
    ]);
  });
  ...
  it("creates a category from the form", async () => {
    request.mockResolvedValueOnce({ id: "category-3", user_id: "user-1", name: "교통", kind: "expense" });
    ...
    await waitFor(() => {
      expect(request).toHaveBeenCalledWith("/categories", {
        method: "POST",
        body: JSON.stringify({ name: "교통", kind: "expense" }),
      });
    });
  ...
  it("fills the form from a category and saves edits", async () => {
    request.mockResolvedValueOnce({ id: "category-1", user_id: "user-1", name: "외식", kind: "expense" });
    ...
    await waitFor(() => {
      expect(request).toHaveBeenCalledWith("/categories/category-1", {
        method: "PUT",
        body: JSON.stringify({ name: "외식", kind: "expense" }),
      });
    });
